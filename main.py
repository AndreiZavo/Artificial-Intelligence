...
class Drone():
    ...
    def moveDSF(self, detectedMap):
        ...
        if self.y < 19 and int(detectedMap.surface[self.x][self.y + 1]) == 0 and self.visited[self.x][self.y + 1] == 0:
            self.stack.append([self.x, self.y + 1])
            newPossibleMove = newPossibleMove + 1
            self.go_back[(self.x, self.y + 1)] = (self.x, self.y)

        elif self.y > 0 and int(detectedMap.surface[self.x][self.y - 1]) == 0 and self.visited[self.x][self.y - 1] == 0:
            self.stack.append([self.x, self.y - 1])
            newPossibleMove = newPossibleMove + 1
            self.go_back[(self.x, self.y - 1)] = (self.x, self.y)

        elif self.x < 19 and int(detectedMap.surface[self.x + 1][self.y]) == 0 and self.visited[self.x + 1][
            self.y] == 0:
            self.stack.append([self.x + 1, self.y])
            newPossibleMove = newPossibleMove + 1
            self.go_back[(self.x + 1, self.y)] = (self.x, self.y)

        elif self.x > 0 and int(detectedMap.surface[self.x - 1][self.y]) == 0 and self.visited[self.x - 1][self.y] == 0:
            self.stack.append([self.x - 1, self.y])
            newPossibleMove = newPossibleMove + 1
            self.go_back[(self.x - 1, self.y)] = (self.x, self.y)

        if newPossibleMove == 0:
            if self.x == self.startX and self.y == self.startY:
                return -1
            else:
                x1 = self.go_back[(self.x, self.y)][0]
                y1 = self.go_back[(self.x, self.y)][1]
